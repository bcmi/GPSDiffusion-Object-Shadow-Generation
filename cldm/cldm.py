...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(self, control_stage_config, control_key, only_mid_control, K, *args, **kwargs):
        ...
        if os.path.exists('./models/pretrained_models/Shadow_cls.pth'):
            self.mask_cls.load_state_dict(torch.load('./models/pretrained_models/Shadow_cls.pth')['net'])
        ...
        if os.path.exists('./models/pretrained_models/Shadow_reg.pth'):
            self.bbx_reg.load_state_dict(torch.load('./models/pretrained_models/Shadow_reg.pth')['net'])
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        mask_reshaped = bbx_region.permute(0, 2, 3, 1)
        batch['zt'] = batch['zt'] * (1 - mask_reshaped)
        batch['zt'] = (batch['zt'].to(torch.float32) / 127.5) - 1.0
